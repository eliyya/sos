...
interface CompletAsyncSelectProps<O, IM extends boolean>
    extends ComponentProps<typeof AsyncSelect<O, IM>> {
    label: string
    children?: ReactNode
    icon?: LucideIcon
    error?: string
...
export function CompletAsyncSelect<
...
interface RetornableCompletAsyncSelectProps<O, IM extends boolean>
    extends CompletAsyncSelectProps<O, IM> {
    originalValue: MultiValue<O> | SingleValue<O>
}
export function RetornableCompletAsyncSelect<
    T extends object = { value: string; label: string },
    IM extends boolean = false,
>({
    children,
    ...
    ref,
    ...props
}: RetornableCompletAsyncSelectProps<T, IM>) {
    const rid = useId()
    const Icon = icon
    const selectRef = useRef<SelectInstance<T, IM>>(null)
    useImperativeHandle(ref, () => selectRef.current!)
