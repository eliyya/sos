...
export function useSearchUsers() {
    ...
    const refreshUsers = useCallback(() => setRefresh(Symbol()), [setRefresh])

    const usersPromise: SearchUsersPromise = useMemo(
        () =>
            fetch(
                `${app.api.pagination.users()}?${createSearchParams(filters)}`,
            )
                .then(res => res.json())
                .catch(() => ({ users: [], count: 0 })),
